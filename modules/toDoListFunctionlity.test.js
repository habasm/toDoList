...
describe('Adding: ', () => {
  ...
  test('Item Added to LocalStorage, Array and DOM', () => {
    expect(myList.addItemToList('Habasm')).toHaveLength(1); // index -> 0
    expect(myList.addItemToList('Hassan')).toHaveLength(2); // index -> 1
...
describe('Removing:', () => {
  ...
  test('Item Removed from LocalStorage, Array and DOM', () => {
    expect(myList.addItemToList('Eating breakfast')).toHaveLength(3);
    expect(myList.addItemToList('Codding with partners')).toHaveLength(4);
    expect(myList.addItemToList('Eating lunches')).toHaveLength(5);
...
describe('Update: ', () => {
  test('Item Updated', () => {
    ...
    expect(myList.editeInputTextTest(pElement, 'Habasm', newItem, 1)).toMatch('Habasm'); // change Haasan to Habasm
